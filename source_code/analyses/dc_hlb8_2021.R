...
syn$serialno <- as.factor(syn$serialno); View(data.frame(table(syn$serialno, useNA="always"))) 
   syn$tract <- ifelse(str_count(syn$tract)==3, paste0("000",syn$tract), 
                    ifelse(str_count(syn$tract)==4, paste0("00",syn$tract), paste0("0",syn$tract)))
   syn$tract <- as.factor(syn$tract)
    syn$puma <- as.factor(paste0("000",syn$puma))
str(syn)
...
for(i in 1:length(index_0)){
  age[index_0[i],] <- sort(unname(unlist(AGE[index_0[i],])), decreasing=FALSE, na.last=NA)
}   
...
#change the negative amounts to 0
DC_taxes$fiitax[DC_taxes$fiitax<0] <- 0
DC_taxes$siitax[DC_taxes$siitax<0] <- 0
  DC_taxes$tfica[DC_taxes$tfica<0] <- 0
#calculate the total tax liability
              syn$total_taxes <- DC_taxes$fiitax+DC_taxes$siitax+DC_taxes$tfica
              syn$total_taxes <- ifelse(syn$total_taxes<0, 0, syn$total_taxes) 
                 syn$hlb_year <- round((syn$hlb_no_tax_yr + syn$total_taxes), 2)
