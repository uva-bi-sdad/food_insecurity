...
#derived variables are added to the DC acs pums data: 
...
dc_pums <- c("00101", "00102", "00103", "00104", "00105", "00106")
dc_vars <- c("WGTP",     #household weight
...
dim(dc_pums); length(unique(dc_pums$SERIALNO)); dim(dc_pums[dc_pums$SPORDER==1,]) #17586 households 
dc_pums <- dc_pums[with(dc_pums, order(PUMA, SERIALNO, SPORDER)),]
View(dc_pums)

#create a variable for the six age categories used to construct the household combination variable
age_cat <-
  ifelse(dc_pums$AGEP>=19, "adult", 
        ifelse((dc_pums$AGEP==0 & dc_pums$NOC>0), "infant",
              ifelse((dc_pums$AGEP>=1 & dc_pums$AGEP<=3), "toddler", 
                    ifelse((dc_pums$AGEP>=4 & dc_pums$AGEP<=5), "preschooler", 
                          ifelse((dc_pums$AGEP>=6 & dc_pums$AGEP<=11), "schooler", "teenager"))))) 
length(age_cat); table(age_cat, useNA="always") 
temp <- data.frame(PUMA=dc_pums$PUMA, SERIALNO=dc_pums$SERIALNO, age_cat=age_cat, NOP=c(rep(1, length(age_cat)))) 
...
hh_size <- temp2$adult+temp2$teenager+temp2$schooler+temp2$preschooler+temp2$toddler+temp2$infant
     NP <- dc_pums$NP[!duplicated(dc_pums$SERIALNO)]
...
length(hh_comb); rm(temp); hh_comb <- as.factor(hh_comb); table(hh_comb, useNA="always")

#create 9 income category groups
#first change the negative income values to 0
                       temp <- dc_pums[!duplicated(dc_pums$SERIALNO),]; dim(temp)
   temp$HINCP[temp$HINCP<0] <- 0
              hh_income_cat <- cut(temp$HINCP, include.lowest=TRUE,
                     breaks=c(0, 14999, 24999, 34999, 49999, 74999, 99999, 149999, 199999, 2000000),
...
HINCP <- dc_pums$HINCP[!duplicated(dc_pums$SERIALNO)]
...
table(index, useNA="always"); length(index)

hh_wt <- dc_pums$WGTP[!duplicated(dc_pums$SERIALNO)]
length(hh_wt)

dc_acs_pums_derived_variables_2021 <- 
  data.frame(puma = temp$PUMA,
             serialno = as.factor(temp$SERIALNO),
             hh_wt = hh_wt,
             hh_comb = as.factor(hh_comb),
...
dim(dc_acs_pums_derived_variables_2021)
#check for errors
View(dc_acs_pums_derived_variables_2021) 

#write.csv(dc_acs_pums_derived_variables_2021, "~/Documents/Documents/Food Insecurity/Data/COL_Fairfax/FINAL/dc_acs_pums_derived_variables_2021.csv")
